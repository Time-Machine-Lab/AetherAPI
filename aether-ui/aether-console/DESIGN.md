...
- **Primary composition**: use a single, centered chat column that matches a familiar LLM assistant layout. Session status, latest plan, and run result should remain embedded in the main narrative column instead of being split into a side rail.
- **Header summary**: keep session metadata compact and supportive. Surface status, session id, plan version, and run status as lightweight tags near the conversation header instead of dedicating a persistent secondary panel.
- **Conversation bubbles**: user messages should use a pale blue family with subtle borders and navy text. Agent messages stay on soft neutral surfaces with thin borders, while plan and run cards are visually stronger assistant responses.
- **Streaming response behavior**: render the latest user turn optimistically when a send starts, and reveal assistant text incrementally in place as the stream arrives. Planning / replying status belongs inside the assistant bubble, not in detached notifications.
...
- **Plan readability**: the current plan card must clearly separate summary, guided clarification controls, legacy clarification questions, category actions, and asset actions. Structured `clarificationItems` render as inline controls inside the plan card, grouped by asset, category, or overall plan context; legacy `clarificationQuestions` only render as question cards when structured items are absent.
