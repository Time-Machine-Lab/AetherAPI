...
<template>
  <div class="min-h-screen">
    <div class="mx-auto flex min-h-screen max-w-6xl flex-col px-4 sm:px-6 lg:px-8">
      ...
      >
        <RouterLink :to="{ name: 'portal-home' }" class="flex items-center gap-3">
        ...
        <div class="flex flex-wrap items-center justify-end gap-2">
          <Button as-child variant="ghost" size="sm">
            <RouterLink :to="{ name: 'portal-home' }">{{ t('portal.nav.home') }}</RouterLink>
          </Button>
          <Button as-child variant="ghost" size="sm">
            <RouterLink :to="{ name: 'portal-workspace' }">{{
              t('portal.nav.workspace')
            }}</RouterLink>
          </Button>
          <Button v-if="!authStore.isAuthenticated" as-child size="sm">
            <RouterLink :to="{ name: 'portal-sign-in' }">{{ t('portal.nav.signIn') }}</RouterLink>
          </Button>
