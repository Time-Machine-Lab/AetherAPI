...
### Requirement: 控制台 import-agent 工作流必须将 thinking 事件与最终 reply 分开展示
当 import-agent stream 返回结构化 `thinking` 事件时，控制台工作流 MUST 将这些事件作为独立的过程摘要展示，而不是把它们拼接进最终助手回复文本。

#### Scenario: 流式会话返回 thinking 事件
...
#### Scenario: thinking 与 reply 同时存在
...
### Requirement: 控制台 import-agent 工作流必须在缺少 thinking 事件时保持向后兼容
thinking 事件是对现有 import-agent stream 的增强能力。当后端未返回 `thinking` 事件时，前端 MUST 继续保持现有 status / reply / session 工作流可用。
...
#### Scenario: thinking 流会在下一次会话开始前被重置
