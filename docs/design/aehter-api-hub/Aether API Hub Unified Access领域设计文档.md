...
Unified Access 在一期新增一条轻量任务查询分支，用于解决上游异步 API “提交后需要按 taskId 查询”的接入缺口。该分支仍属于 Unified Access，而不是平台管理接口或日志查询接口。

推荐外部入口为：

- `POST /api/v1/access/{apiCode}`：保持现有提交/调用行为
- `GET /api/v1/access/{apiCode}/tasks/{taskId}`：按同一 API 资产的异步任务配置查询上游任务

任务查询必须复用普通 Unified Access 的前置约束：

1. 校验 `X-Aether-Api-Key` 并解析 Consumer Context。
2. 解析 `apiCode` 对应的已发布、未删除目标资产。
3. 校验当前调用方是资产所有者或拥有有效订阅。
4. 确认目标资产声明了完整的 `async_task_config`。
5. 按 `{taskId}` 渲染任务查询 URL 模板。
6. 通过现有下游转发边界执行请求，并继续使用平台代理路由、上游鉴权注入、超时和执行失败分类。
7. 记录可观测调用事实。
