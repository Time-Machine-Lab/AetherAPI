...
Unified Access 的异步任务查询也是一次完整的上游调用事实，因此仍然进入 `api_call_log`，不新增平台任务生命周期表。日志需要能区分普通提交/调用与任务查询调用，推荐通过 `access_channel = UNIFIED_ACCESS_TASK_QUERY` 或等价结果上下文字段表达。

该设计保持三条边界：

- Observability 记录“任务查询这次调用”的事实，而不是记录“任务本身”的生命周期。
- 不保存上游 taskId 的归属映射、状态历史或最终结果缓存。
- 不提供当前用户任务列表；后续若需要，应由独立任务中心能力定义存储表、状态机和查询接口。
