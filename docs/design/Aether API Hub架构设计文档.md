...
管理员在 Hub 中录入一个新的 API 资源，填写分类、描述、上游地址、鉴权方式、请求示例、响应示例等信息。如果该 API 属于 AI API，还需要额外标记供应商、模型名或能力标签、是否支持流式等元数据。录入完成后，API 进入可展示状态。

这个场景验证的是 Hub 的核心内容底座是否成立，即平台能否承载 API 资源本身，而不是依赖代码硬编码维护。

#### 场景二：API 浏览与接入

开发者通过最小展示页浏览 API 列表和详情，识别 API 的分类、类型、用途、调用方式和凭证获取方式。对于 AI API，开发者还可以明确看到其 AI 属性和后续扩展能力。

这个场景验证的是 Hub 是否真正具备“Hub 形态”，而不仅仅是一个后台系统。
...
第一期不追求复杂前台，而是通过最小可用页面验证 Hub 产品形态。
...
这样做的目标不是提前做重权限系统，而是为后续控制台、分析、限额、计费和安全治理留出稳定基础。
...
负责 Aether API Hub 与普通 API 管理后台的差异化能力。
...
1. 管理员通过后台接口或管理页面提交 API 基础配置
2. API Catalog 模块校验并保存 API 元数据
3. 若为 AI API，则同时保存 AI-ready 扩展字段
4. 平台将 API 状态置为草稿或启用
5. Hub Portal 从 Catalog 模块读取数据进行列表和详情展示
...
1. 开发者访问 Hub 最小展示页
2. Hub Portal 查询 API Catalog 中的 API 列表和详情
