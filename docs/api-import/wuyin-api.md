...
**Wan2.6 生成视频API接口，支持角色扮演、音画同步、多镜头生成及声音驱动等功能
**

**接口地址：** [https://api.wuyinkeji.com/api/async/video_wan2.6](https://api.wuyinkeji.com/api/async/video_wan2.6)

**返回格式：** application/json

**请求方式：** **HTTP**POST

**请求示例：** `https://api.wuyinkeji.com/api/async/video_wan2.6?key=你的密钥`
...
| 名称            | 必填 | 类型   | 示例值                                                                                                                                             | 说明                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------- | ---- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| prompt          | 是   | string | 一只猫咪跳舞                                                                                                                                       | 文本提示词，不超过200字，超过则自动截断                                                                                                                                                                                                                                                                                                                                                             |
| negative_prompt | 否   | string | 不要出现动作变形                                                                                                                                   | 反向提示词，描述不希望在视频中看到的内容，不超过500个字符                                                                                                                                                                                                                                                                                                                                           |
| audio_url       | 否   | string | ``                                                                                                                                                 | 音频文件URL                                                                                                                                                                                                                                                                                                                                                                                         |
| firstFrameUrl   | 否   | string | ``                                                                                                                                                 | 首帧图，支持URL或 Base64 编码数据                                                                                                                                                                                                                                                                                                                                                                   |
| size            | 否   | string | 1280*720                                                                                                                                           | 指定生成的视频分辨率，格式为宽*高。1280*720：16:9。720*1280：9:16。960*960：1:1。1088*832：4:3。832*1088：3:4，1920*1080： 16:9。1080*1920： 9:16。1440*1440： 1:1。1632*1248： 4:3。1248*1632： 3:4                                                                                                                                                                                      |
| duration        | 否   | string | 5                                                                                                                                                  | 生成视频的时长，单位为秒。可选值为5、10、15。默认值为5                                                                                                                                                                                                                                                                                                                                              |
| prompt_extend   | 否   | string | true                                                                                                                                               | 是否开启prompt智能改写。开启后大模型对输入prompt进行智能改写，会增加耗时。true：默认值，开启智能改写。false：不开启智能改写                                                                                                                                                                                                                                                                         |
| shot_type       | 否   | string | single                                                                                                                                             | 指定生成视频的镜头类型，即视频是由一个连续镜头还是多个切换镜头组成。生效条件：仅当prompt_extend: true 时生效。参数优先级：shot_type > prompt。例如，若 shot_type设置为single，即使 prompt 中包含生成多镜头视频，模型仍会输出单镜头视频。可选值：single：默认值，输出单镜头视频。multi：输出多镜头视频。                                                                                             |
| watermark       | 否   | string | false                                                                                                                                              | 是否添加水印标识，水印位于视频右下角，文案固定为AI生成。false：默认值，不添加水印。true：添加水印。                                                                                                                                                                                                                                                                                                 |
| seed            | 否   | string | 0                                                                                                                                                  | 随机数种子，取值范围为[0-7]。未指定时，系统自动生成随机种子。若需提升结果的可复现性，建议固定seed值。注意，由于模型生成具有概率性，即使相同 seed，也不能保证每次生成结果完全一致。                                                                                                                                                                                                                  |
| urls            | 否   | array  | ["https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/xxx.mp4", "https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/xxx.jpg"] | 上传的参考文件 URL 数组，支持传入视频和图像。用于提取角色形象与音色（如有），以生成符合参考特征的视频。每个 URL 可指向 一张图像 或 一段视频：图像数量：0～5视频数量：0～3总数限制：图像 + 视频 ≤ 5传入多个参考文件时，按照数组顺序定义角色的顺序。即第 1 个 URL 对应 character1，第 2 个对应 character2，以此类推。每个参考文件仅包含一个主体角色。例如 character1 为小女孩，character2 为闹钟。`` |
...
数字人 API 接口，上传人物正面视频 + 音频，即可一键驱动数字人精准对口型。

**接口地址：** [https://api.wuyinkeji.com/api/async/video_digital_humans](https://api.wuyinkeji.com/api/async/video_digital_humans)

**返回格式：** application/json

**请求方式：** **HTTP**POST

**请求示例：** `https://api.wuyinkeji.com/api/async/video_digital_humans?key=你的密钥`
...
| 名称      | 必填 | 类型   | 示例值           | 说明                                      |
| --------- | ---- | ------ | ---------------- | ----------------------------------------- |
| videoName | 是   | string | ``               | 名称                                      |
| audioUrl  | 否   | string | xxx.com/test.mp3 | 音频地址：公网可访问链接                  |
| videoUrl  | 否   | string | xxx.com/test.mp4 | 视频地址：公网可访问链接，时长不得低于10S |
...
**接口地址：** [https://api.wuyinkeji.com/api/async/audio_tts](https://api.wuyinkeji.com/api/async/audio_tts)
...
| language_boost | 否   | string | `` | 语种 默认auto增强对指定的小语种和方言的识别能力，设置后可以提升在指定小语种/方言场景下的语音表现。如果不明确小语种类型，则可以选择"auto"，模型将自主判断小语种类型。支持以下取值：``'Chinese', 'Chinese,Yue', 'English', 'Arabic', 'Russian', 'Spanish', 'French', 'Portuguese', 'German', 'Turkish', 'Dutch', 'Ukrainian', 'Vietnamese', 'Indonesian', 'Japanese', 'Italian', 'Korean', 'auto' |
...
**接口地址：** [https://api.wuyinkeji.com/api/voice/composite](https://api.wuyinkeji.com/api/voice/composite)

**返回格式：** application/json

**请求方式：** **HTTP**POST

**请求示例：** `https://api.wuyinkeji.com/api/voice/composite`
...
| 名称           | 必填 | 类型           | 示例值                                                           | 说明                                                                                                                                                                                                                                                                                                                                                                                                    |
| -------------- | ---- | -------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| text           | 是   | string         | 你好，我是你的克隆声音，我会十七国语音，希望未来可以和你好好相处 | 生成音频后朗诵的内容                                                                                                                                                                                                                                                                                                                                                                                    |
| voice_id       | 是   | string         | male-qn-qingse                                                   | 音色ID系统音色在文档下方                                                                                                                                                                                                                                                                                                                                                                                |
| speed          | 否   | float`` | 1.0                                                              | 范围[0.5,2]，默认值为1.0,生成声音的语速，可选，取值越大，语速越快                                                                                                                                                                                                                                                                                                                                       |
| vol            | 否   | float          | 1.0``                                                     | 范围（0,10]，默认值为1.0``生成声音的音量，可选，取值越大，音量越高                                                                                                                                                                                                                                                                                                                               |
| language_boost | 否   | string         | ``                                                        | 语种默认为null增强对指定的小语种和方言的识别能力，设置后可以提升在指定小语种/方言场景下的语音表现。如果不明确小语种类型，则可以选择"auto"，模型将自主判断小语种类型。支持以下取值：``'Chinese', 'Chinese,Yue', 'English', 'Arabic', 'Russian', 'Spanish', 'French', 'Portuguese', 'German', 'Turkish', 'Dutch', 'Ukrainian', 'Vietnamese', 'Indonesian', 'Japanese', 'Italian', 'Korean', 'auto' |
...
| msg           | string | 状态信息       |
| data          | object | 扩展字段       |
| data.url      | string | 合成的音频地址 |
| data.duration | float  | 音频时长       |
| data.subtitle | array  | 音频字幕       |
| data.words    | int    | 字数           |
