 ...
 */
public final class ImportAgentPlannerJsonSupport {
    ...
    public static String buildAgentMessage(String providerName, ImportAgentPlanModel plan) {
        String prefix = providerName == null || providerName.isBlank() ? "规划器" : providerName;
        if (plan.isExecutable()) {
            return prefix + " 已准备好计划版本 " + plan.getVersion() + "，等待确认。";
        }
        if (plan.getClarificationQuestions().isEmpty()) {
            return prefix + " 已准备好未完整的计划版本 " + plan.getVersion() + "。";
        }
        return prefix + " 已准备好计划版本 " + plan.getVersion() + "，还需要补充："
                + String.join("；", plan.getClarificationQuestions());
    }

    public static JsonNode parseJsonCandidate(String candidate) {
