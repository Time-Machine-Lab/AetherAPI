# catalog-asset-delete-lifecycle Specification

## Purpose

Define stability requirements for owner-scoped API asset soft deletion, ensuring deletion persists through the `is_deleted` marker, is not modeled as unpublish, hides deleted assets from all active reads, and rejects further lifecycle operations on deleted assets.
