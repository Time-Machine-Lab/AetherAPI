...
- 明确需要按开发规范维护顶层权威设计产物：`docs/api/api-credential.yaml` 作为 `ApiCredentialController.java` 对应的单一接口契约文件，`docs/sql/api_credential.sql`、`docs/sql/consumer_identity.sql`、`docs/sql/user_consumer_mapping.sql` 作为三张表一一对应的表结构文件；后续生成必须使用 `tml-docs-spec-generate` 技能，其中 API 文档使用 API 模板，SQL 文档使用 SQL 模板。
