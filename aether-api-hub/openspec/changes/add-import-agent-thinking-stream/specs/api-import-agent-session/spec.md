...
### 需求：Import-agent 流式接口必须在最终 session 快照前暴露结构化 thinking 事件
系统必须允许 import-agent 的创建会话流式接口和追加轮次流式接口，在最终 `session` 快照发出前输出结构化 `thinking` 事件，用于描述安全的规划进度摘要。

#### 场景：创建会话时先返回 planning thinking，再返回最终 session
...
#### 场景：追加轮次时返回 clarification 或 review thinking
...
#### 场景：旧客户端可以忽略新增的 thinking 事件
- **WHEN** 某个客户端只理解现有的 `status`、`message`、`session`、`error` 和 `done` 事件
- **THEN** 流式契约必须保持为加法式扩展，使该客户端即使忽略 `thinking` 事件，也不影响其消费最终 session 结果

### 需求：Thinking 事件必须输出脱敏摘要，而不是原始模型 chain-of-thought
系统必须将 import-agent 的 `thinking` 事件视为脱敏后的推理摘要。系统不得通过流式契约暴露原始模型 chain-of-thought、携带 secret 的字段或未脱敏的内部 payload。

#### 场景：带 secret 的鉴权字段不会作为 thinking 内容输出
- **WHEN** planner 或 subagent 的推理过程涉及 `authConfig`、API Key、Bearer Token 或其他携带 secret 的上游配置
- **THEN** 流中只能输出诸如“上游鉴权配置仍然缺失”之类的脱敏摘要，而不能暴露原始 secret 值

#### 场景：prompt 与 provider payload 不会通过 thinking 事件输出给客户端
...
#### 场景：原始模型 reasoning 会被抑制或替换为稳定摘要
